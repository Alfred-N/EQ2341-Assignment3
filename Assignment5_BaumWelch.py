...
covs = np.array([[[1, 0],[0, 10**(-3)]] for i in range(n_states)])
...
Astar = np.random.uniform(size=(n_states,n_states))
for i in range(n_states):
    Astar[i,:] = Astar[i,:]/sum(Astar[i,:])

meansstar = np.zeros([n_states,2])
covsstar =  np.array([[[1, 0],[0, 10**(-3)]] for i in range(n_states)])
Bstar = np.array([multigaussD(meansstar[i],covsstar[i]) for i in range(n_states)])

hm_learn = HMM(qstar, Astar, Bstar)


print("Running the Baum Welch Algorithm...")
hm_learn.baum_welch(song_1, 20, prin=1, uselog=False)
...
print("likelihood of song 1 = ", log_likelihood1)
print("likelihood of song 2 = ", log_likelihood2)
print("likelihood of song 3 = ", log_likelihood3)
